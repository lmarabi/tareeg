...
"""
import Numeric, PropertiedClasses
...
class MaskedArray (PropertiedClasses.PropertiedClass):
    ...
    def __init__(self, data, typecode=None, 
        ...
        if need_data_copied:
            self._data = Numeric.array(c, typecode=tc, copy=1)
            self.savespace(ss)
        else:
            self._data = c
    ...
    def _set_shape (self, name, newshape):
    ...
    def _set_imaginary (self, name, value):
        "x.imaginary = value"
        y = self.imaginary
        y[...] = value
...
"""
 ...
 %(data)s)""" 
    ...
    def __float__(self):
        ...
        x = filled(self, 0)
        return masked_array(self.astype(Float), self.mask())[...]

    def __int__(self):
        "Convert self to int."
        x = filled(self, 0)
        return masked_array(self.astype(Int), self.mask())[...]
...
# setting a value changes the mask to match the value in those locations.
    ...
    def __idiv__(self, other): 
        ...
        if self._mask is None:
            dm = domain_safe_divide()(self._data, filled(other,0))
            self._data /= f
            self._mask = dm
            self._shared_mask = dm is not None
        else:
            mo = getmask(other)
...
PropertiedClasses.set_property (MaskedArray, 'shape', 
                                MaskedArray._get_shape,
                                MaskedArray._set_shape,
                                nodelete=1
             )
PropertiedClasses.set_property (MaskedArray, 'flat', 
                              MaskedArray._get_flat, 
                              MaskedArray._set_flat, 
                              nodelete=1
                             )
PropertiedClasses.set_property (MaskedArray, 'real', 
                              MaskedArray._get_real, 
                              MaskedArray._set_real,
                              nodelete = 1
                             ) 
PropertiedClasses.set_property (MaskedArray, 'imaginary', 
                              MaskedArray._get_imaginary, 
                              MaskedArray._set_imaginary,
                              nodelete = 1
                             ) 
PropertiedClasses.set_property (MaskedArray, 'imag', 
                              MaskedArray._get_imaginary, 
                              MaskedArray._set_imaginary,
                              nodelete = 1
                             ) 
...
PropertiedClasses.set_property(MaskedScalar, 'shape', nowrite=1)
PropertiedClasses.set_property(MaskedScalar, 'imaginary', nowrite=1)
PropertiedClasses.set_property(MaskedScalar, 'imag', nowrite=1)
PropertiedClasses.set_property(MaskedScalar, 'real', nowrite=1)
PropertiedClasses.set_property(MaskedScalar, 'flat', nowrite=1)
masked = MaskedScalar()
...
def average (a, axis=0, weights=None, returned = 0):
    ...
    if axis is None:
        if weights is None:
            w = masked_array(Numeric.choose(ma, (1.0,0.0)), ma)
        else:
            w = array(filled(weights, 0.0), Float, mask = ma, copy = 0)
        n = add.reduce(a.flat*w.flat)
        d = add.reduce(w.flat) 
    else:
        ash = a.shape
        if ash == ():
        ...
        if weights is None:
            ...
            wv = filled(weights, 0.0) 
            wsh = wv.shape
            if wsh == ():
                wsh = (1,)
            if wsh == ash:
                w = array(wv, Float, mask=ma, copy=0)
                n = add.reduce(a*w, axis)
                d = add.reduce(w, axis) 
            elif wsh == (ash[axis],):
                ni = ash[axis]
                ...
                w = eval ("wv["+ repr(tuple(r)) + "] * masked_array(ones(ash, Float), ma)")
                n = add.reduce(a*w, axis)
                d = add.reduce(w, axis)
            else:
    ...
    result = n/d
    # This routine is a very hard routine to get right -- PFD
    # Keep these debug prints as comments for future suffering.
    #print "a", a
    #print "ma", ma
    #print "w", w
    #print "n", n
...
def masked_inside(x, v1, v2, copy=1):
    ...
    if v2 < v1:
        ...
        v1 = t
    d = filled(x)
    c = logical_and(less_equal(d, v2), greater_equal(d, v1))
    m = mask_or(c, getmask(x))
    return array(d, mask = c, copy=copy)
